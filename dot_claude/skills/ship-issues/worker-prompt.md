...
Fill every `<...>` slot: `<owner/repo>`, `<base-branch>`, `<verification-command>`,
`<toolchain-note>`, and the four forge slots come from the repository facts gathered in
step 1; `<N>`, `<other-worker-files>`, and `<issue-specific-note>` from steps 2–6.
`<base-branch>` is the default branch unless step 12 stacked this Issue on an unmerged
dependency, in which case it is that dependency's branch. Drop a slot's line entirely
when it does not apply — do not pass the placeholder through.

The forge slots on `github` are: `<forge-cli>` = `gh`, `<issue-view-command>` =
`gh issue view <N> -R <owner/repo> --comments`, `<pr-list-command>` = `gh pr list`,
`<pr-create-command>` = `gh pr create`. On `gitlab` they are `glab` and the equivalents
`~/.claude/forge/gitlab.md` gives (`glab issue view <N> -R <owner/repo> --comments`,
`glab mr list`, `glab mr create ... --yes`). The worker reads `issue-pr/SKILL.md`, whose
own preamble makes it run `forge-detect.sh` and read `gitlab.md` itself; the slots only
keep this prompt's wording consistent with that.
...
あなたは Issue #`<N>` だけを実装する worker です。
...
`<pr-create-command>`）。読めない場合は止めて報告してください。
...
3. 対象は Issue #`<N>` のみ（`<issue-view-command>`）。
...
4. リポジトリの `AGENTS.md` / `CLAUDE.md` / `.claude/rules/` に従う。
   ...
   （`git log`、`<pr-list-command>` で確認。慣例が無ければ日本語）。最終報告は日本語。
...
6. リポジトリホスティング（Issue / PR）の操作はすべて `<forge-cli>` で行う。`<forge-cli>`
   が使えない、または未認証なら、別の手段に逃げず止まって報告する。
7. `<toolchain-note>`
...
11. push してから `<pr-create-command>` で PR を 1 つだけ作る。本文は
